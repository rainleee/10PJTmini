		...
		//==> jQuery 적용 추가된 부분
		 $(function() {
			...
			//==> DOM Object GET 3가지 방법 ==> 1. $(tagName) : 2.(#id) : 3.$(.className)
		 	...
		 	$( ".Depth03:contains('구매목록조회')" ).on("click" , function() {
				//Debug..
				//alert(  $( ".Depth03:contains('구매목록조회')" ) );
		 		$(window.parent.frames["rightFrame"].document.location).attr("href","/purchase/listPurchaseProduct");
			}); 
		 	
		 	$( ".Depth03:contains('판매물품관리')" ).on("click" , function() {
	...
	<td valign="top"> 
		<table  border="0" cellspacing="0" cellpadding="0" width="159">
			...
			<c:if test="${ !empty user && user.role == 'user'}">
				...
				<tr>
					<td class="Depth03">
						<!-- <a href="/purchase/listPurchaseProduct"  target="rightFrame">구매목록조회</a> -->
						구매목록조회
					</td>
				</tr>
			</c:if>
			...
			<c:if test="${ !empty user && user.role == 'admin'}">
				<tr>
				<td class="Depth03">
					<!-- <a href="/purchase/listSale"  target="rightFrame">판매 된 물품관리</a> -->
					판매물품관리
				</td>
			</tr>
			</c:if>
